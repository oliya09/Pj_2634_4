...
class CNNModel(nn.Module):
    def __init__(self):
        ...
        self.fc2 = nn.Linear(64, 1)  # Без сигмоиды!
    ...
    def forward(self, x):
        ...
        x = self.dropout(x)
        x = self.fc2(x)
        return x  # Без sigmoid — она в loss
...
if __name__ == '__main__':
    ...
    criterion = nn.BCEWithLogitsLoss()  # ✅ безопасно для autocast
        ...
        with torch.no_grad():
            for batch_x, batch_y in val_loader:
                ...
                with autocast(device_type=device.type):
                    ...
                    loss = criterion(outputs, batch_y)
                val_loss += loss.item()
                probs = torch.sigmoid(outputs)  # ✅ sigmoid для метрик
